...
def store_predictions(df, benchmark_type, model_name, mode):
    predictions = []
    try:
        start_index = 0
...
def run_benchmark_store_answers(model_name, benchmark_type, df, mode):
    try:
        print('store prediction started')
        store_predictions(df, benchmark_type, model_name, mode)
    except Exception as e:
